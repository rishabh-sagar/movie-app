...
// import "../assests/instagram-logo.svg" from '../assests/instagram-logo.svg'
...
function ActorDetails({ actor, loading, onClose }) {
	return (
		<div className="fixed w-screen h-screen z-40 px-4 py-16 overflow-auto bg-black bg-opacity-75">
			{loading && <Loader />}
			{!loading &&
				<div className="relative p-2 w-full max-w-3xl m-auto flex flex-col sm:flex-row rounded-2xl  text-white bg-[#1C1C1C]">
					<span 
					...
					<div className="flex-1 p-2">
						...
						<div className="flex space-x-2 items-center my-2">
							{actor.links.imdb && <IconLink icon={"/assets/imdb-logo.svg"} link={actor.links.imdb} />}
							{actor.links.instagram && <IconLink icon={"/assets/instagram-logo.svg"} link={actor.links.instagram} />}
							{actor.links.twitter && <IconLink icon={"/assets/twitter-logo.svg"} link={actor.links.twitter} />}
							{actor.links.facebook && <IconLink icon={"/assets/facebook-logo.svg"} link={actor.links.facebook} />}
						</div>
