...
TMDB = {
	...
	getTitle: async(type, id) => {
		const title = await axios.get(`${API_BASE_URL}/${type}/${id}?api_key=${TMDB_API_KEY}`)
		console.log(title)
		if (title.data) {
			title.data = {
				...title.data,
	...
	getActors: async(type, id) => {
		const credits = await axios.get(`${API_BASE_URL}/${type}/${id}/credits?api_key=${TMDB_API_KEY}`)
		let actors
